...
class ResidualCorrector:
    ...
    def apply(self, frame: pd.DataFrame, pred: np.ndarray) -> np.ndarray:
        ...
        adjustment = self.strength * self.corrections.reindex(idx).fillna(0.0).to_numpy(dtype=float)
        if self.mean_neutral and len(adjustment) > 0:
            adjustment = adjustment - float(np.mean(adjustment))
        return np.clip(values + adjustment, 0.0, None)

    def summary(self) -> dict[str, object]:
        n_corrections = 0 if self.corrections is None else int(len(self.corrections))
        return {
            "group": self.group,
            "columns": list(self.columns),
            "smoothing": float(self.smoothing),
            "strength": float(self.strength),
            "mean_neutral": bool(self.mean_neutral),
...
def fit_residual_corrector(
    ...
    if group == "none" or not group_columns or strength <= 0.0:
        return ResidualCorrector(group="none", strength=0.0, mean_neutral=mean_neutral)
