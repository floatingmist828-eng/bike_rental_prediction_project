...
class EventProfileTests(unittest.TestCase):
    ...
    def test_score_rebound_fit_weather3_overcount_uses_stronger_rain_cut(self) -> None:
        test_df = pd.DataFrame(
            {
                "dteday": [
                    "2012-10-02",
        ...
        adjusted, meta = apply_event_adjustments(test_df, pred, profile="score_rebound_fit_weather3_overcount")

        np.testing.assert_allclose(adjusted, np.array([91.25, 70.0, 165.0, 400.0]))
        self.assertEqual(meta["profile"], "score_rebound_fit_weather3_overcount")
        self.assertEqual(meta["changed_rows"], 3)

    def test_overcount_hum_rush_profile_adjusts_conditions_without_overriding_storm(self) -> None:
        test_df = pd.DataFrame(
            {
                "dteday": [
                    "2012-10-02",
                ...
                "hr": [12, 8, 18, 12, 13],
                "workingday": [0, 1, 1, 1, 1],
                "weathersit": [1, 1, 1, 3, 1],
                "hum": [0.90, 0.50, 0.50, 0.90, 0.50],
            }
        )
        ...
        adjusted, meta = apply_event_adjustments(
            test_df,
            pred,
            profile="score_rebound_fit_weather3_overcount_hum_rush",
        )

        np.testing.assert_allclose(adjusted, np.array([105.0, 203.0, 297.0, 140.0, 500.0]))
        self.assertEqual(meta["profile"], "score_rebound_fit_weather3_overcount_hum_rush")
        self.assertEqual(meta["changed_rows"], 4)

    def test_overcount_late_hour_profile_multiplies_hour_factors_after_condition_sets(self) -> None:
        test_df = pd.DataFrame(
            {
                "dteday": [
                    "2012-08-02",
                ...
                "weathersit": [1, 1, 1, 3, 3],
                "hum": [0.50, 0.50, 0.90, 0.50, 0.90],
            }
        )
        pred = np.array([100.0, 200.0, 300.0, 400.0, 500.0])
        ...
        adjusted, meta = apply_event_adjustments(
            test_df,
            pred,
            profile="score_rebound_fit_weather3_overcount_late_hour",
        )
        ...
        self.assertEqual(meta["profile"], "score_rebound_fit_weather3_overcount_late_hour")
        self.assertEqual(meta["changed_rows"], 5)
